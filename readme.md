...
Исправьте функцию `index_page()` следующим образом:
...
1. В файле `index.html` сразу после заголовка добавьте следующие теги:
...
```powershell
git add .
git commit -m "Add form to sum two numbers"
```
